  ...
  outputs = { self, nixpkgs }:
    let
      ...
      dependencies = pkgs: with pkgs; [
        gtk4 glib libxkbcommon wayland pipewire
        gst_all_1.gstreamer gst_all_1.gst-plugins-base
      ];
      runtime = pkgs: with pkgs; [
        sway dbus firefox gnome-text-editor wayvnc
        gst_all_1.gst-plugins-base gst_all_1.gst-plugins-good pipewire
      ];
    in {
      ...
      devShells = eachSystem (system:
        ...
        in {
          default = pkgs.mkShell {
            nativeBuildInputs = with pkgs; [ cargo rustc rustfmt clippy pkg-config nixfmt ];
