#!/usr/bin/env bash
# @file run-niri.sh
# @brief 在私有 D-Bus 与嵌套 niri 中执行整机验收
...
if [[ "${1:-}" == inner ]]; then
	...
	export COMPUTER_USE_NIRI_TEST=1 GTK_A11Y=none GDK_BACKEND=wayland GSK_RENDERER=cairo
...
if [[ "${1:-}" != outer ]]; then
	...
	exec env -u WAYLAND_DISPLAY -u DISPLAY -u NIRI_SOCKET -u AT_SPI_BUS_ADDRESS HOME="$test_runtime/home" XDG_CACHE_HOME="$test_runtime/cache" XDG_RUNTIME_DIR="$test_runtime" XDG_STATE_HOME="$test_runtime/state" \
