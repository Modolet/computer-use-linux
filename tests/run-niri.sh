...
if [[ "${1:-}" == inner ]]; then
	...
	dbus-update-activation-environment WAYLAND_DISPLAY XDG_CURRENT_DESKTOP NIRI_SOCKET
	unset NO_AT_BRIDGE
	...
	"$TEST_ATSPI_REGISTRY" &
	set +e
	cargo test --test niri -- --ignored --nocapture --test-threads=1
	status=$?
...
WLR_BACKENDS=headless WLR_HEADLESS_OUTPUTS=1 WLR_RENDERER=pixman sway --config "$XDG_RUNTIME_DIR/sway.config" >"$XDG_RUNTIME_DIR/sway.log" 2>&1 &
outer_pid=$!
inner_pid=''
trap '[[ -z "$inner_pid" ]] || kill "$inner_pid" 2>/dev/null || true; kill "$outer_pid" 2>/dev/null || true; wait 2>/dev/null || true' EXIT
for _ in {1..100}; do
...
LIBGL_ALWAYS_SOFTWARE=1 niri --config "$XDG_RUNTIME_DIR/niri.kdl" -- bash "$0" inner >"$XDG_RUNTIME_DIR/niri.log" 2>&1 &
inner_pid=$!
for _ in {1..6000}; do
